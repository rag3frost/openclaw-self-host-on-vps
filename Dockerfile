...
RUN apt-get update   && DEBIAN_FRONTEND=noninteractive apt-get install -y --no-install-recommends     ca-certificates     curl     git     gosu     procps     build-essential     zip   && rm -rf /var/lib/apt/lists/*   && curl -LsSf https://raw.githubusercontent.com/NVIDIA/OpenShell/main/install.sh | OPENSHELL_INSTALL_DIR=/usr/local/bin sh   && curl -LsSf https://astral.sh/uv/install.sh | UV_INSTALL_DIR=/usr/local/bin sh

# Install Python 3.12 using uv and create venv
RUN uv python install 3.12     && python3.12 -m venv /opt/openspace-venv

# Upgrade pip in the venv
RUN /opt/openspace-venv/bin/pip install --upgrade pip setuptools wheel

RUN groupadd -g 1001 sandbox &&     useradd -u 1001 -g sandbox -m -s /bin/bash sandbox

RUN npm install -g openclaw@v2026.3.24 mcporter

# Clone OpenSpace repository
RUN git clone https://github.com/HKUDS/OpenSpace.git /opt/OpenSpace

# Install OpenSpace in the venv
RUN /opt/openspace-venv/bin/pip install -e /opt/OpenSpace

# Make OpenSpace venv available on PATH
ENV PATH="/opt/openspace-venv/bin:/usr/local/bin:/home/linuxbrew/.linuxbrew/bin:/home/linuxbrew/.linuxbrew/sbin:/usr/bin:/bin:/usr/local/sbin:/usr/sbin:/sbin"
ENV OPENSPACE_WORKSPACE="/opt/OpenSpace"

# Register OpenSpace MCP server with mcporter
RUN mcporter config add openspace --command "openspace-mcp"     --env OPENSPACE_HOST_SKILL_DIRS="/data/workspace/skills"     --env OPENSPACE_WORKSPACE="/opt/OpenSpace"

WORKDIR /app
COPY requirements.txt .
RUN /opt/openspace-venv/bin/pip install --no-cache-dir -r requirements.txt
...
USER openclaw
RUN NONINTERACTIVE=1 /bin/bash -c "#!/bin/bash
...
then
  ...
  if [[ "${#chgrps[@]}" -gt 0 ]]
  then
    execute_sudo "${CHGRP[@]}" "${USER}" "${chgrps[@]}"
  fi
else
...
then
  ...
  execute_sudo "${CHGRP[@]}" "${USER}" "${mkdirs[@]}"
fi
...
EOS"
ENV PATH="/home/linuxbrew/.linuxbrew/bin:/home/linuxbrew/.linuxbrew/sbin:/usr/local/bin:/home/linuxbrew/.linuxbrew/bin:/home/linuxbrew/.linuxbrew/sbin:/usr/bin:/bin:/usr/local/sbin:/usr/sbin:/sbin"
...
EXPOSE 8080
HEALTHCHECK --interval=30s --timeout=5s --start-period=10s   CMD curl -f http://localhost:8080/setup/healthz || exit 1
USER root
